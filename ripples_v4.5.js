...
function touchRipple ( e ) {
	let x, y ;
	if(e.type == 'touchstart' ) {
	var touch = e.originalEvent.touches[0] || e.originalEvent.changedTouches[0];
		x = Math.floor( touch.pageX / pixel ) ;
		y = Math.floor( touch.pageY / pixel ) ;
	} else if (e.type == 'mousedown' ) {
		x = Math.floor( e.clientX / pixel ) ;
		y = Math.floor( e.clientY / pixel ) ;
	}
	if ( e.target.hasPointerCapture (e.pointerId) ) {
      e.target.releasePointerCapture (e.pointerId) ;
	}
	let newDrop = new Drop ( new Tetron ( x , y ) , ripples.width , ripples.height ) ;
	ripples.surface.addDrop ( newDrop ) ;
	return ;
};
...
window.addEventListener( 'pointerdown' , touchRipple ) ;
window.addEventListener( 'mousedown' , touchRipple ) ;
window.addEventListener( 'touchstart' , touchRipple ) ;
